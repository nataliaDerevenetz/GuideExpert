...
fun ExcursionPOJO.toExcursion() = Excursion(id,title, description,images.map { it.toImage() },idSort)

fun Excursion.toExcursionPOJO() = ExcursionPOJO(id,title, description,images.map{ it.toImagePOJO()},idSort)

fun ExcursionPOJO.toExcursionSearchEntity() = ExcursionSearchEntity(id,title, description,idSort,images.map { it.toImage() })

fun ExcursionPOJO.toExcursionFilterEntity() = ExcursionFilterEntity(id,title, description,idSort,images.map { it.toImage() })


fun ExcursionPOJO.toExcursionSearchWithData() = ExcursionSearchWithData(
    ExcursionSearchEntity(id,title, description,idSort,images.map{it.toImage()}),
    images.map{ it.toImagePreviewSearchEntity() }
)

fun ExcursionPOJO.toExcursionFilterWithData() = ExcursionFilterWithData(
    ExcursionFilterEntity(id,title, description,idSort,images.map{it.toImage()}),
